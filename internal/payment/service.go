...
func NewPaymentService(pr *PaymentRepository, domain string, key string) *PaymentService {
...
func (ps *PaymentService) GetGatewayPaymentStatus(ctx context.Context, paymentID uuid.UUID, amount float64) (PakasirStatusResponse, error) {
	reqUrl := fmt.Sprintf("https://app.pakasir.com/api/transactiondetail?project=%s&amount=%.0f&order_id=%s&api_key=%s",
		ps.depoDomain, amount, paymentID.String(), ps.apiKey)
...
func (ps *PaymentService) processNewPayment(ctx context.Context, orderData model.Order, paymentMethod string) (model.Payment, *shared.ErrorResponse) {
	...
	if err != nil {
		ps.repo.MarkAsExpired(ctx, newPaymentData.ID)
...
func (ps *PaymentService) AddTransaction(
	ctx context.Context,
	userID, orderID uuid.UUID,
	paymentMethod string,
) (model.Payment, *shared.ErrorResponse) {

	orderData, err := ps.repo.GetOrderDataById(ctx, userID, orderID)
	if err != nil {
		if errors.Is(err, ErrOrderNotFound) {
			return model.Payment{}, shared.NewErrorResponse(
				404,
				"order data does not exist",
			)
		}

		return model.Payment{}, shared.NewErrorResponse(
			500,
			"something went wrong while checking the order data, try again",
		)
	}

	// hanya order pending yang boleh bikin payment
	if orderData.Status != "pending" {
		return model.Payment{}, shared.NewErrorResponse(
			409,
			"order has already been processed, completed, or cancelled",
		)
	}

	latestPayment, err := ps.repo.GetLatestPayment(
		ctx,
		userID,
		orderData.ID,
	)

	// payment belum ada -> bikin baru
	if err != nil {
		if errors.Is(err, ErrPaymentNotFound) {
			return ps.processNewPayment(
				ctx,
				orderData,
				paymentMethod,
			)
		...
		return model.Payment{}, shared.NewErrorResponse(
			500,
			"something went wrong while checking payment data, try again",
		)
	}
	...
	case "unpaid":
		// kalau sudah lewat expired time → mark expired lalu create baru
		if latestPayment.ExpiredAt != nil && time.Now().After(*latestPayment.ExpiredAt) {

			if err := ps.repo.MarkAsExpired(ctx, latestPayment.ID); err != nil {
				return model.Payment{}, shared.NewErrorResponse(
					500,
...
func (ps *PaymentService) WebHookVerification(ctx context.Context, req PakasirStatusResponse, paymentID uuid.UUID) (model.Payment, *shared.ErrorResponse) {
	...
	// bandingin
	// kalo beda
	if responseData.Transaction.OrderID != req.Transaction.OrderID {
		fmt.Println("error unauthenticated why tho? : ", req)
